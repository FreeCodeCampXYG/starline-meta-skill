...
- Scope: absorb its useful publishing behavior into `starline-meta-skill` so creation and publication no longer require a second skill
- License: MIT
...
- Legacy Qiaomu profile/QR asset injection with stable markers (studied but removed from Starline 3.0.0)
- `npx skills add --list` discovery and real temporary installation
- optional canonical `~/.agents/skills/<name>` synchronization

### Adapt

- Starline 3.0.0 removed the prior author's bundled Profile assets and CLI flags; generated README ownership now comes from package metadata.
...
- Scope: improve `starline-meta-skill` with a repeatable discover-compare-synthesize gate
...
- Keep the lighter gate ladder and Chinese-first packaging while borrowing rigorous evaluation patterns only where they improve the output contract.
