...
Creating a Starline skill invoked both `starline-meta-skill` and a generic `skill-creator`. This produced two competing authoring workflows and unnecessary instruction loading.
...
- Make `starline-meta-skill` the single authoring authority once triggered.
- Keep domain prior-art discovery inside `starline-meta-skill`; do not install a separate discovery skill.
...
For “生成一个 SEO skill”, the selected workflow should be `starline-meta-skill` using its own catalog-discovery method; neither a separate creator nor a separate discovery skill should appear in the execution chain.
...
The package validator also assumed every generated Starline skill must contain the meta skill's own routing tokens and Yao upstream credit. Validation is now package-aware: meta-only routing checks apply only to `starline-meta-skill`, and README upstream credit is required only when `manifest.json` declares `upstream_inspiration`.
