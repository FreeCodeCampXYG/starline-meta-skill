#!/usr/bin/env python3
"""Export a compact Skill IR document from a Starline skill package."""
...
SCHEMA_VERSION = "2.0.0-starline-lite"
...
def build_ir(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "intent": {
            ...
            "target_users": intent.get("target_users", ["Starline operator"]),
            "inputs": intent.get("inputs", []),
            "outputs": intent.get("outputs", []),
            "exclusions": intent.get("exclusions", []),
            "starline_defaults": manifest.get("starline_defaults", {}),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export compact Skill IR for a Starline skill.")
