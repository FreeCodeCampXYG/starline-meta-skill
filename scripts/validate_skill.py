#!/usr/bin/env python3
"""Validate the lightweight Starline skill package contract."""
...
def discover_skill_entrypoints(root: Path) -> list[Path]:
    ...
    return sorted(entries, key=lambda path: (len(path.parts), path.as_posix().lower()))
...
def validate(root: Path) -> dict[str, Any]:
    ...
    if root.name == "starline-meta-skill":
        ...
        for relative in ("scripts/research_prior_art.py", "scripts/release_check.py", "scripts/publish_skill.py"):
            if not (root / relative).is_file():
                failures.append(f"starline-meta-skill missing built-in factory script: {relative}")
    ...
    if skill_md.exists():
        ...
        if frontmatter.get("name") == "starline-meta-skill":
            for token in ("skill", "starline", "workflow"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate a Starline skill package.")
