...
修改人：**墨点星痕**｜英文名：**starline**  原始作者:**乔木向阳**
...
```text
用 Starline 元 Skill，把这套工作流做成一个可复用的 Skill；
先研究同类热门 Skill，完成触发评测和安全检查，然后发布到 GitHub。
```

它会自己完成：**需求收敛 → 同类检索 → 取长避短 → Skill 设计 → 触发评测 → 格式校验 → README → API 泄露检查 → PR → Release → npx 安装验证**。

**v3.0.2 Windows 发布修复：** 发布器和发布门禁都能正确解析 `npx.cmd`、读取 UTF-8 命令输出，并在临时用户目录完成干净安装验证；PR 使用 rebase merge 保留 `Starline` 作者身份。

### 命名体系

- 新建 skill 默认使用 `starline-` 前缀。
...
| 能力 | 普通“生成 SKILL.md” | starline-meta-skill |
|---|---:|---:|
...
| 区分设计优势、已验证优势和待验证假设 |  | ✓ |
| 校验目录、版本、上下文预算与递归发现 |  | ✓ |
| README、动态 Owner 与 MIT License 自动准备 |  | ✓ |
| Secret / API 泄露扫描 |  | ✓ |
| 功能分支、PR、检查、Release |  | ✓ |
...
截至 2026-08-04，我扫描并去重了本机 Codex 会话、创建交接和 prior-art 报告。迁移前能确认有明确创建或实质重构证据的旧 `qiaomu-*` Skill 共 **28 个**；其中 **18 个已有公开仓库**。这些名称属于历史事实，不代表当前 Starline 默认命名。
...
| [`qiaomu-meta-skill`](https://github.com/joeseesun/qiaomu-meta-skill)（迁移前上游） | 原元 Skill 的公开历史记录；当前本地包已迁移为 `starline-meta-skill` |
...
Starline 元 Skill 不会看到排行榜第一名就照搬。它会从 skills.sh、SkillsMP 与 GitHub 找出“流行度锚点、可信来源、互补专家”，阅读源文件后再决定保留、改造、拒绝或原创。
...
本地 Codex 已从以下目录发现该 Skill：

```text
~/.codex/skills/starline-meta-skill
```

只安装这个 Skill：

公开发布后，将 `<github-user>` 替换为实际账号：

```bash
npx skills add <github-user>/starline-meta-skill --skill starline-meta-skill
```

验证：

```bash
test -f ~/.agents/skills/starline-meta-skill/SKILL.md
python3 ~/.agents/skills/starline-meta-skill/scripts/validate_skill.py \
  ~/.agents/skills/starline-meta-skill
```
...
`starline-meta-skill` turns prompts, SOPs, transcripts, scripts, and existing skills into researched, evaluated, installable agent-skill packages.
...
After publishing under your own account:

```bash
npx skills add <github-user>/starline-meta-skill
```
...
The project is intentionally fork-friendly: install it, run a real workflow, then adapt Starline's defaults to your own judgment, tools, style, and evaluation boundary.
